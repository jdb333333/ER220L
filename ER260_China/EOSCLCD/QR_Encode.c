...
int m_ncDataBlock;
BYTE m_byBlockMode[MAX_BLOCKMODE];
BYTE m_nBlockLength[MAX_BLOCKMODE];
...
struct GRAPH_DEFINE graph_info_QRCODE = {0};

#define XYSCALE		4//二维码点放大倍数1,2,4,8
#define BITMAX	(8/XYSCALE)

#define QRCODE_Y 	80		//TFT二维码显示坐标y

int  QRWidth;//二维码的宽度(字节数)
int  QRSize;//二维码的总字节数

#if (XYSCALE==1)
const BYTE BYTEBit[BITMAX]={0x80,0x40,0x20,0x10,0x08,0x04,0x02,0x01};
#elif (XYSCALE==2)
const BYTE BYTEBit[BITMAX]={0xc0,0x30,0x0c,0x03};
#elif (XYSCALE==4)
const BYTE BYTEBit[BITMAX]={0xf0,0x0f};
#elif (XYSCALE==8)
const BYTE BYTEBit[BITMAX]={0xff};
#endif

//---------------------------------------------------------------------------
//压缩二维码
/**
 * 生成字符串qrcode_data对应的二维码
 * 然后将二维码点阵数据压缩存储到m_byAllCodeWord中
 *
 *  QRWidth:为图片的宽度(字节数)
 *  QRSize:为图片数据的大小(字节数)
 * @author EutronSoftware (2016-06-22)
 *
 * @param qrcode_data
 */
...
{
	int i,j;
	int QR1stByte,m_bDataEncoded;
    BYTE QRByte;

    QRSize=0;
    QRWidth=0;

    if (EncodeData((char *)qrcode_data) && m_nSymbleSize<=240)//图像的最大宽度为240点
    {
      m_bDataEncoded = m_nSymbleSize;
      memset(m_byAllCodeWord,0,MAX_ALLCODEWORD);
      for(j=0;j<m_bDataEncoded;j++)
      {
          QRByte=0;
          QR1stByte=QRSize;//记录第一个位置
          for(i=0;i<m_bDataEncoded;i++)
          {
              if(m_byModuleData[i][j]==1)
              {
                  m_byAllCodeWord[QRSize] |= BYTEBit[QRByte];
              }
          ...
          if (QRByte)
              QRSize++;
          if (QRWidth==0)
              QRWidth = QRSize;//图片的宽度
  #if (XYSCALE>1)
          //进行Y方向放大
          for(i=0;i<XYSCALE-1;i++)
          {
              memcpy(m_byAllCodeWord+QRSize,m_byAllCodeWord+QR1stByte,QRWidth);
              QRSize+=QRWidth;
          }
  #endif
      }
      graph_info_QRCODE.cGrapCols=QRWidth;
      graph_info_QRCODE.cLabels =0;
      graph_info_QRCODE.cPrintAt =(PRTLEN*12/8-QRWidth)/2;
      graph_info_QRCODE.cWidthAChar=8;
      graph_info_QRCODE.cHighAChar=24;
      graph_info_QRCODE.cByteALine = 24;
      graph_info_QRCODE.cGrapSize =QRSize;
    }
    else
    {//生成的二维码错
        graph_info_QRCODE.cGrapCols=0;
        graph_info_QRCODE.cLabels =0;
        graph_info_QRCODE.cPrintAt =0,
        graph_info_QRCODE.cWidthAChar=8;
        graph_info_QRCODE.cHighAChar=24;
        graph_info_QRCODE.cByteALine = 24;
        graph_info_QRCODE.cGrapSize =0;
    }
}
//ccr2016-06-22<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
#endif
